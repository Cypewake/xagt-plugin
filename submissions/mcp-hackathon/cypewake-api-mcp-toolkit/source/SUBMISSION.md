...
- **Health-check URL**：`<DEPLOY_URL>/api/health`
...
- **Deploy**：`uvicorn demo_app:app --host 0.0.0.0 --port $PORT`，并设置环境变量 `REVIEW_COMMIT=<REVIEW_COMMIT>`
- **Version binding**：部署服务通过环境变量 `REVIEW_COMMIT` 暴露审查 commit；`/api/health` 与 `/.well-known/xagent-verification.json` 回传该 commit。

部署服务必须暴露：

```json
// GET <DEPLOY_URL>/api/health
{"status":"ok","commit":"<REVIEW_COMMIT>"}
```

```json
// GET <DEPLOY_URL>/.well-known/xagent-verification.json
...
- **Health-check 结果**：`status=ok`，`commit=<REVIEW_COMMIT>`。
